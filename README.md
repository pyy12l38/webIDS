...
- `ids_backend_django`：Django API 后端
- `ids_frontend_vue`：Vue 3 + TypeScript + Vite 前端

这部分用于前后端分离升级实验，目前不覆盖主运行链路。

## 说明

当前主系统已经不再使用旧的日志采集与旁路采集链路，默认以 **Proxy 单模式** 运行为准。  
如果后续继续扩展，建议仍以“监听端口 + 后端端口”的代理接入方式作为新增站点的统一方案。
